...
def store_kill(values):
	if not values or not isinstance(values, dict):
		return
	fields = ['attacker_uid', 'victim_uid', 'attacker_name', 'victim_name']
	for field in fields:
		if field not in values or not str(values[field]).strip():
			return
	attacker_id = str(values.get('attacker_id', values.get('attacker_uid', ''))).strip()
	victim_id = str(values.get('victim_id', values.get('victim_uid', ''))).strip()
	attacker_name = re.sub(r'\(\d*\)', '', str(values['attacker_name']).strip()).strip()
	victim_name = re.sub(r'\(\d*\)', '', str(values['victim_name']).strip()).strip()
	if not attacker_name or not attacker_id or not victim_name or not victim_id:
		return
...
def webui():
	...
	<head>
		...
		input, button { margin: 0.3em 0.3em 0.3em 0; font-size: 1.1em; padding: 0.3em 0.5em; border-radius: 4px; border: 1px solid #444; background: #232323; color: #eee; }
		input:focus, button:focus { outline: 2px solid #ff7575; }
		...
		table { border-collapse: collapse; width: auto; min-width: 0; background: #232323; font-size: 1.1em; border-radius: 6px; overflow: hidden; }
		...
		.pagination button { background: #232323; color: #eee; border: 1px solid #444; padding: 0 6px; margin-right: 0.2em; cursor: pointer; font-size: 0.85em; height: 1.5em; min-width: unset; border-radius: 4px; }
		.pagination button:disabled { color: #666; border-color: #333; cursor: default; }
		.pagination input[type=number] { width: 2.2em; padding: 0 2px; font-size: 1em; background: #232323; color: #eee; border: 1px solid #444; text-align: center; height: 1.5em; border-radius: 4px; }
		</style>
	</head>
	<body>
		...
		<div id="leaderboard">
			...
			<div style="margin-bottom:0.15em; display: flex; align-items: center; gap: 0.3em;">
				...
				<label style="font-size:0.9em; display: flex; align-items: center;">
					...
					<input type="number" id="top-perpage" min="1" max="100" value="10" style="width:3.0em;padding:0 1px;font-size:0.85em;height:1.2em;" aria-label="Players per page">
					<input type="number" id="top-page" min="1" value="1" aria-label="Page number">
				</label>
